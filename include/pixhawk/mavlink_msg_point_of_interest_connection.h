...
#define MAVLINK_MSG_ID_162_LEN 54
...
#define MAVLINK_MESSAGE_INFO_POINT_OF_INTEREST_CONNECTION { \
	...
	{  { "type", NULL, MAVLINK_TYPE_UINT8_T, 0, 0, offsetof(mavlink_point_of_interest_connection_t, type) }, \
         ...
         { "name", NULL, MAVLINK_TYPE_INT8_T, 25, 29, offsetof(mavlink_point_of_interest_connection_t, name) }, \
         } \
}
...
static inline uint16_t mavlink_msg_point_of_interest_connection_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
						       uint8_t type, uint8_t color, uint8_t coordinate_system, uint16_t timeout, float xp1, float yp1, float zp1, float xp2, float yp2, float zp2, const int8_t *name)
{
	...
	_mav_put_int8_t_array(buf, 29, name, 25);
	...
	packet.yp2 = yp2;
	packet.zp2 = zp2;
	mav_array_memcpy(packet.name, name, sizeof(int8_t)*25);
        memcpy(_MAV_PAYLOAD_NON_CONST(msg), &packet, 54);
#endif

	msg->msgid = MAVLINK_MSG_ID_POINT_OF_INTEREST_CONNECTION;
	return mavlink_finalize_message(msg, system_id, component_id, 54);
}
						           ...
						           uint8_t type,uint8_t color,uint8_t coordinate_system,uint16_t timeout,float xp1,float yp1,float zp1,float xp2,float yp2,float zp2,const int8_t *name)
{
	...
	msg->msgid = MAVLINK_MSG_ID_POINT_OF_INTEREST_CONNECTION;
	return mavlink_finalize_message_chan(msg, system_id, component_id, chan, 54);
}
...
static inline void mavlink_msg_point_of_interest_connection_send(mavlink_channel_t chan, uint8_t type, uint8_t color, uint8_t coordinate_system, uint16_t timeout, float xp1, float yp1, float zp1, float xp2, float yp2, float zp2, const int8_t *name)
{
	...
	_mav_put_float(buf, 25, zp2);
	_mav_put_int8_t_array(buf, 29, name, 25);
	_mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_POINT_OF_INTEREST_CONNECTION, buf, 54);
#else
	mavlink_point_of_interest_connection_t packet;
	packet.type = type;
	packet.color = color;
	...
	packet.zp2 = zp2;
	mav_array_memcpy(packet.name, name, sizeof(int8_t)*25);
	_mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_POINT_OF_INTEREST_CONNECTION, (const char *)&packet, 54);
#endif
}
 ...
 */
static inline uint16_t mavlink_msg_point_of_interest_connection_get_name(const mavlink_message_t* msg, int8_t *name)
{
	return _MAV_RETURN_int8_t_array(msg, name, 25,  29);
}
...
#else
	memcpy(point_of_interest_connection, _MAV_PAYLOAD(msg), 54);
#endif
}

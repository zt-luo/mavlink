...
#define MAVLINK_MSG_ID_151_LEN 255
...
#define MAVLINK_MESSAGE_INFO_WATCHDOG_PROCESS_INFO { \
	...
	{  { "watchdog_id", NULL, MAVLINK_TYPE_UINT16_T, 0, 0, offsetof(mavlink_watchdog_process_info_t, watchdog_id) }, \
         ...
         { "name", NULL, MAVLINK_TYPE_INT8_T, 100, 4, offsetof(mavlink_watchdog_process_info_t, name) }, \
         { "arguments", NULL, MAVLINK_TYPE_INT8_T, 147, 104, offsetof(mavlink_watchdog_process_info_t, arguments) }, \
...
static inline uint16_t mavlink_msg_watchdog_process_info_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
						       uint16_t watchdog_id, uint16_t process_id, const int8_t *name, const int8_t *arguments, int32_t timeout)
{
	...
	_mav_put_int8_t_array(buf, 4, name, 100);
	_mav_put_int8_t_array(buf, 104, arguments, 147);
	...
	mav_array_memcpy(packet.name, name, sizeof(int8_t)*100);
	mav_array_memcpy(packet.arguments, arguments, sizeof(int8_t)*147);
...
{
	return _MAV_RETURN_int8_t_array(msg, name, 100,  4);
}
...
{
	return _MAV_RETURN_int8_t_array(msg, arguments, 147,  104);
}

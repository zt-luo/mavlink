...
#define MAVLINK_MSG_ID_114_LEN 32
...
static inline void mavlink_msg_global_vision_position_estimate_send(mavlink_channel_t chan, uint64_t usec, float x, float y, float z, float roll, float pitch, float yaw)
{
	...
	_mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_GLOBAL_VISION_POSITION_ESTIMATE, buf, 32);
#else
	mavlink_global_vision_position_estimate_t packet;
	packet.usec = usec;
	packet.x = x;
	...
	_mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_GLOBAL_VISION_POSITION_ESTIMATE, (const char *)&packet, 32);
#endif
}

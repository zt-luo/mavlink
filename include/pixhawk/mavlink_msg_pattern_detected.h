...
#define MAVLINK_MSG_ID_160_LEN 106
...
#define MAVLINK_MESSAGE_INFO_PATTERN_DETECTED { \
	...
	{  { "type", NULL, MAVLINK_TYPE_UINT8_T, 0, 0, offsetof(mavlink_pattern_detected_t, type) }, \
         ...
         { "file", NULL, MAVLINK_TYPE_INT8_T, 100, 5, offsetof(mavlink_pattern_detected_t, file) }, \
...
static inline uint16_t mavlink_msg_pattern_detected_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
						       uint8_t type, float confidence, const int8_t *file, uint8_t detected)
{
	...
	_mav_put_int8_t_array(buf, 5, file, 100);
	...
	mav_array_memcpy(packet.file, file, sizeof(int8_t)*100);
 ...
 */
static inline uint16_t mavlink_msg_pattern_detected_get_file(const mavlink_message_t* msg, int8_t *file)
{
	return _MAV_RETURN_int8_t_array(msg, file, 100,  5);
}
